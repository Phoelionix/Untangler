...
def get_cross_conf_nonbonds(pdb_file_path,out_file,verbose,use_cdl):
    ...
    def run_probe_clashscore(probe_clashscore_manager, pdb_string):
        ...
        #probe_cmd_and_args=f'{probe_cmd} -u -q -mc -het -once {add_vdw_arg}  "ogt0 not water" "ogt0" - ' 
        probe_cmd_and_args=f'{probe_cmd} -u -q -mc -het -once {add_vdw_arg}  "ogt10 not water" "ogt10" - ' 
        ...
        # with open("clashes_out_raw_data.txt","w+") as f:
        #     f.write(probe_out)
    ...
    for f in filtered_dicts:
        ...
        debug_found=False
        for coord in coords:
            if round(coord[0],1)==46.0:
                debug_found=True
                break
        if debug_found:
            print(site_labels)
            print(coords,vdw_sum,symop_str)
            print("---")
